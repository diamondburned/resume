% Resume partially based off of Jake Gutierrez's template.
...
\section{EDUCATION}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading
      ...
      {<.studyType> in <.area>}{\small{<.startDate>} - \small{<.endDate>}}
    <end>
...
\section{WORK EXPERIENCE}
  ...
  \resumeSubHeadingListStart
    <range $i, $item := .work>
    </* $same := and (gt $i 0) (eq (index $.work (sub $i 1)).company .company) */>
    <$same := false>
    \vspace{<if $same> -6pt <else> -2pt <end>} % compensate
    \item
      \begin{tabular*}{0.965\textwidth}[t]{l@{\extracolsep{\fill}}r}
        <if not $same>
        \textbf{<.company>} & \small{<.location>} \\
        <end>
        \textit{\small{<.position>}} & \textit{\small{<.startDate> - <.endDate>}} \\
      \end{tabular*}
    \resumeItemListStart
      <range .highlights>
      \resumeItem{<.>}
...
\section{LEADERSHIP}
  \resumeSubHeadingListStart
    \item
      \begin{tabular*}{0.965\textwidth}[t]{l @{ \ } l @{\extracolsep{\fill}} r}
        <range .leadership>
        \vspace{2pt}
        \textbf{<.company>} & {| \ \textit{<.position>}} &
        \small{<.startDate><if (has . "endDate")> - <.endDate><end>} \\
        <end>
      \end{tabular*}
  \resumeSubHeadingListEnd
...
\section{SKILLS}
  \resumeSubHeadingListStart
    \item{
      ...
      \begin{tabular*}{0.965\textwidth}[t]{p{0.11\linewidth}@{ \ }p{0.89\linewidth}}
        <range .skills>
